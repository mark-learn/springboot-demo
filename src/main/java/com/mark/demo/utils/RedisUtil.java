...
public class RedisUtil {
     ...
     * @param key   键
     * @param value 值
     * @param ttl  时间(秒) ttl要大于0 如果ttl小于等于0 将设置无限期
     ...
     * @return true成功 false失败
     */
    public boolean hmset(String key, Map map, long ttl) {
        try {
            redisTemplate.opsForHash().putAll(key, map);
            if (ttl > 0) {
                expire(key, ttl);
     ...
     * @param item  项
     * @param value 值
     * @param ttl  时间(秒) 注意:如果已存在的hash表有时间,这里将会替换原有的时间
     * @return true 成功 false失败
     */
    public boolean hset(String key, String item, Object value, long ttl) {
        try {
            redisTemplate.opsForHash().put(key, item, value);
            if (ttl > 0) {
                expire(key, ttl);
     ...
     * @return 成功个数
     */
    public long sSetAndTime(String key, long ttl, Object... values) {
        try {
     ...
     * @return
     */
    public boolean lSet(String key, List value, long ttl) {
        try {
            redisTemplate.opsForList().rightPushAll(key, value);
            if (ttl > 0) {
                expire(key, ttl);
    ...
    public Set<String> keys(String keyPrefix) {
        ...
        try {
            ...
            return (Set<String>) execute;
        } catch (Throwable ex) {
            log.error("redis keys error", ex);
            return new HashSet<>();
        }
    }
